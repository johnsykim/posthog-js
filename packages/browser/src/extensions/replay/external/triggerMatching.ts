...
export function triggerGroupsMatchSessionRecordingStatus(
    ...
    // If any group is activated (conditions met) but sample didn't hit, still record
    // This ensures trigger activation always records, just without the "sampled" flag
    if (anyGroupActivated) {
        return ACTIVE
    }
    ...
    // All groups are either disabled or conditions not met
    return DISABLED
}
