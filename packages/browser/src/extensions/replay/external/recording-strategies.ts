...
export class V1RecordingStrategy implements RecordingStrategy {
    private _triggerMatching: TriggerStatusMatching | undefined
    ...
    getStatus(context: RecordingStrategyContext): SessionRecordingStatus {
        const config = context.remoteConfig
        const matchFunction =
            config?.triggerMatchType === 'any' ? anyMatchSessionRecordingStatus : allMatchSessionRecordingStatus
    ...
    makeSamplingDecisions(sessionId: string): void {
        ...
        const storedIsSampled = this._getIsSampled(sessionId)
        const storedSessionId = this._getStoredSessionId()
        const sessionIdChanged = !isNull(storedSessionId) && storedSessionId !== sessionId

        // If false (explicitly not sampled for this session), keep it false
        // Note: if storedSessionId is null, it means the value is bare `false` (not tied to a session),
        // which should be preserved as "not sampled" for this session
        if (storedIsSampled === false && !sessionIdChanged) {
            return
        }
    ...
    hasPendingTriggers(sessionId: string): boolean {
        return this._triggerMatching?.triggerStatus(sessionId) === TRIGGER_PENDING
    }
    ...
    private _getIsSampled(sessionId: string): boolean | null {
        const currentValue = this._instance.get_property(SESSION_RECORDING_IS_SAMPLED)
        if (currentValue === true) {
            return null
        }
        return currentValue === false ? false : typeof currentValue === 'string' ? currentValue === sessionId : null
    }
...
export class V2TriggerGroupStrategy implements RecordingStrategy {
    ...
    makeSamplingDecisions(sessionId: string): void {
        const sessionIdChanged = this._getStoredSessionId() !== sessionId
        ...
        for (const matcher of this._triggerGroupMatchers) {
            ...
            // Validate group ID is safe for use as persistence key suffix
            if (!/^[a-zA-Z0-9_-]+$/.test(groupId)) {
                logger.warn('[SessionRecorder] Invalid group ID for persistence - skipping group', { groupId })
                continue
            }
            ...
            // Make decision if session changed or no stored decision
            const makeDecision = sessionIdChanged || !isBoolean(storedDecision)
    ...
    updateActiveTriggers(): void {
        ...
        for (const matcher of this._triggerGroupMatchers) {
            const group = matcher.group
            const groupId = group.id
            const triggerStatus = matcher.triggerStatus(
                this._instance.sessionManager!.checkAndGetSessionAndWindowId().sessionId
            )
    ...
    private _getStoredSessionId(): string | null {
        // Get the session ID from any of the sampling keys (they should all match)
        for (const matcher of this._triggerGroupMatchers) {
            const storageKey = SESSION_RECORDING_TRIGGER_V2_GROUP_SAMPLING_PREFIX + matcher.group.id
            const storedValue = this._instance.get_property(storageKey)
